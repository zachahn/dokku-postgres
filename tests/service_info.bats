...
teardown() {
  ...
  dokku --force "$PLUGIN_COMMAND_PREFIX:destroy" test_with_underscores || true
}
...
@test "($PLUGIN_COMMAND_PREFIX:info) success" {
  ...
  assert_contains "${lines[*]}" "postgres://postgres:$password@dokku-bitnami-postgres-l:5432/l"
}
...
@test "($PLUGIN_COMMAND_PREFIX:info) replaces underscores by dash in hostname" {
  ...
  assert_contains "${lines[*]}" "postgres://postgres:$password@dokku-bitnami-postgres-test-with-underscores:5432/test_with_underscores"
...
@test "($PLUGIN_COMMAND_PREFIX:info) success with flag" {
  ...
  local password="$(sudo cat "$PLUGIN_DATA_ROOT/l/PASSWORD")"
  assert_output "postgres://postgres:$password@dokku-bitnami-postgres-l:5432/l"
