...
Vagrant.configure(2) do |config|
  ...
  config.vm.define "default", primary: true do |vm|
    ...
    vm.vm.provision :shell, :inline => "apt -q update && apt -y -qq install git software-properties-common build-essential"
    vm.vm.provision :shell, :inline => "cd /vagrant && DOKKU_VERSION=#{DOKKU_VERSION} make setup"
    vm.vm.provision :shell, :inline => "usermod -aG docker vagrant"
